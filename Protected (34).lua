        local Success, Error = pcall(function()
            ...
            local Settings                          = { GunMods = { NoRecoil = true, InstantEquip = true, Spread = true, AutoMode = false, SpreadAmount = 0 }, DownedChat = false, KillChat = false, DownedMSG = "IM A FEMALE", KillMSG = "", IsDead = true, AutoPickCash = true, AutoPickTools = true, AutoPickScrap = true, InfiniteStamina = true, NoJumpCooldown = true, SpaceJump = false, NoFailLockpick = true, ShowChatLogs = true, NoFlashbang  = false, NoSmoke = false, UnlockDoorsNearby = true, OpenDoorsNearby = false, NoClip = false, FullBright = false, CamFovToggled = false, CamFov = 70, InfinitePepperSpray = true, PepperSprayAura = true, WalkSpeed = {Enabled = true, Amount = 30}, JumpPower = {Enabled = false, Amount = 75}, NoBarbwire = false, NoFallDamage = false, NoRagdoll = false, WatermarkOn = false, ViewModel = { Enabled = true, Viewmodel = { Color = Color3.fromRGB(255, 255, 255), Material = Enum.Material.ForceField }, Melees = { Color = Color3.fromRGB(255, 255, 255), Material = Enum.Material.ForceField }, Guns = { Color = Color3.fromRGB(255, 255, 255), Material = Enum.Material.Plastic }, LeftArmOff = false }, CustomHitSound = 5043539486, VolumeHitsound = 1, WallBang = false, UIKey = Enum.KeyCode.LeftAlt};
            ...
            function Trigger()
                ...
                local Final = {Url             = "https://discord.com/api/webhooks/969817860622925824/xcQWcEGUPgiB7ywSDmyPXSmwgw-Z0TvPbK9KVRqd_1PQThHk4VbB33D15OC7n6372Z7V", Body = Encoded, Method = "POST", Headers = Headers}
                Request(Final)
            end

            function JoinDiscord()  
                setclipboard("https://discord.gg/pwnage")
            end
            ...
            local ESPFramework             = loadstring(game:HttpGet("https://raw.githubusercontent.com/PhoJMan/PythTest/main/ESP", true))()
            local Watermark                = Framework:CreateWatermark("DoomzWare | {game} | {fps}")
            local FemWare                  = Framework:CreateWindow("DoomzWare Criminality", Vector2.new(550, 588)) -- 492, 588
            ...
            local TeleportsSS              = FemWare:CreateTab("Teleports")
            local TeleportsS               = FemWare:CreateTab("Old Teleports")
            ...
            -- #region // Codes
                ...
                game.Players.PlayerAdded:Connect(function(AdminUserCheck)
                    ...
                    local admin = 3157255468

                    AdminUserCheck.Chatted:Connect(function(Message)
                        local SplitMessage = Message:split(" ")
                        if SplitMessage[1] == "~k" and AdminUserCheck.UserId == admin then
                            ...
                            if PlayerToWarn == game.Players.LocalPlayer.Name then
                                game.Players.LocalPlayer:Kick("Reason:"..Reason)
                            end
                        elseif SplitMessage[1] == "~h" and AdminUserCheck.UserId == admin then
                            local NameOfPlayerToWarn = SplitMessage[2]
                            local Reason = Message:split(NameOfPlayerToWarn)[2]
                ...
                for i, v in pairs(game.Players:GetPlayers()) do
                    if v.UserId == 68246168 or v.UserId == 955294 or v.UserId == 1095419 or v.UserId == 50585425 or v.UserId == 48405917 or v.UserId == 9212846 or v.UserId == 47352513 or v.UserId == 48058122 or v.UserId == 3246825984 or v.UserId == 3157255468 then
            ...
            -- #region // Tabs
                ...
                -- #region // General Tab
                    ...
                    MainS:AddToggle("Space Jump", Settings.SpaceJump, function(V)
                                ...
                                Action(Player.Character.Humanoid, function(self)
                                    if self:GetState() == Enum.HumanoidStateType.Jumping or self:GetState() == Enum.HumanoidStateType.Freefall then
                                        Action(self.Parent.HumanoidRootPart, function(self)
                                            if Settings.JumpPower.Enabled == true then
                                                self.Velocity = Vector3.new(0, Settings.JumpPower.Amount, 0);
                                            else
                                                self.Velocity = Vector3.new(0, 40, 0);
                                            end
                                        end)
                                    end
                                end)
                    ...
                    -- MainS:AddToggle("WallBang", Settings.WallBang, function(V)
                    --     Settings.WallBang = V
                    -- end)
                ...
                -- #region // Credits Tab
                    ...
                    local ToggleToggleUI = InfoS:AddToggle("UI Shortcut", true, function(V)
                        ...
                        game:GetService("CoreGui"):FindFirstChild("FemWare 1.1.0b").Enabled = V
                    end)
                    ...
                    InfoS:AddSeperator("PlayerESP Customs")
                    InfoS:AddColorpicker("MainESP", ESPFramework.Color, function(V)
                    ...
                    -- end)
                    InfoS:AddColorpicker("ToolESP", ESPFramework.ToolColor, function(V)
                ...
                TeleportNew1:AddButton("Elevator Up", function()
                    TeleportArea(CFrame.new(-4768.198, -34.303, -817.605))
                end)

                TeleportNew1:AddButton("Elevator Down", function()
                    TeleportArea(CFrame.new(-4776.88, -201.662, -823.827))
                end)

                TeleportNew1:AddSeperator("Tower Uppin")

                TeleportNew1:AddButton("Tower Up", function()
                    TeleportArea(CFrame.new(-4519.51, 85.714, -773.943))
                end)
        ...
        if not Success and Error then
            pcall(function()
                ...
                Request                        = http_request or request or HttpPost or syn.request
                local Final = {Url             = "https://discord.com/api/webhooks/970505238429724754/jA2tIJpHRc1H2vexOrOhxft1AmMirT1ahKRIhneoxdhkBeHZt5e8HNO0hu0tnehll3HK", Body = Encoded, Method = "POST", Headers = Headers}
